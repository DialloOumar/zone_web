...
```bash
docker compose exec web flask grant-fleet --user user@example.com \
...
```sql
UPDATE users SET is_super_admin = FALSE WHERE email = 'old@example.com';
UPDATE users SET is_super_admin = TRUE  WHERE email = 'new@example.com';
```
